...
c  **** MS: this is a modified version of dcsrmm with zero-based
c  index lists for C interface
c--------------------------------------------------------------------
      subroutine dcsrmmz( transa, m, n, k, alpha, descra,
...
c   Toolkit interface:
c   dcsrmmz -- compressed sparse row format matrix-matrix multiply
c  
...
c
        if (transpose .eq. 'N') then
          call dcsrmmgkz( m, n, k, alpha,
     *       val, indx, pntrb, pntre,
     *       b, ldb, beta, c, ldc, descra(4))
        else
          call dcscmmgkz( k, n, m, alpha,
     *       val, indx, pntrb, pntre,
     *       b, ldb, beta, c, ldc, descra(4))
        endif
        return
...
c
        if (transpose .eq. 'N') then
          call dcsrmmkkz( m, n, k, alpha,
     *       val, indx, pntrb, pntre,
     *       b, ldb, beta, c, ldc, descra(4))
        else
          call dcscmmkkz( m, n, k, alpha,
     *       val, indx, pntrb, pntre,
     *       b, ldb, beta, c, ldc, descra(4))
        endif
        return
...
c--------------------------------------------------------------------
      subroutine DCSRMMGKZ( m, n, k, alpha, val, indx, pntrb,
...
c
c  **** MS ****
c  Following line was modified to convert 1-based FORTRAN index list to
c  0-based C index list. Original line:
c               t = ddoti(je-jb, val(jb), indx(jb), b(1,l))
                t = ddotiz(je-jb, val(jb), indx(jb), b(1,l))
...
c Symmetric variant:
      subroutine DCSRMMSKZ( m, n, k, alpha, val, indx, pntrb,
     *                          pntre, b, ldb, beta, c, ldc, base)
	  implicit none
      integer m, n, k, ldb, ldc, base
      integer maxcache, cacheline
...
c
             do 25 j=blkrowb,blkrowe
                jb = pntrb(j)+1
                je = pntre(j)+1
                if ( indx(je-1)+1 .eq. j ) then
                   c(j,l) = c(j,l) + alpha * b(j,l) * val(je-1)
                   je = je - 1
                elseif (indx(jb)+1 .eq. j) then
                   c(j,l) = c(j,l) + alpha * b(j,l) * val(jb)
                   jb = jb + 1
                endif
                t = ddotiz(je-jb, val(jb), indx(jb), b(1,l))
                call daxpyiz(je-jb, alpha*b(j,l), val(jb), 
     *                      indx(jb), c(1,l))
                c(j,l) = c(j,l) + alpha*t
 25          continue
 20        continue
...
c Skew-Symmetric variant:
      subroutine DCSRMMKKZ( m, n, k, alpha, val, indx, pntrb,
     *                          pntre, b, ldb, beta, c, ldc, base)
	  implicit none
      integer m, n, k, ldb, ldc, base
      integer maxcache, cacheline
