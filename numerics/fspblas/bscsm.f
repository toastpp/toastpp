...
c
c   Toolkit interface:
c   dbscsm -- block sparse column format triangular solve
c  
...
c
      ...
      if ( (transa .ne. 0) .and. (transa .ne. 1) ) then
         ...
         print *,'Insufficient work space for BSCSM.'
         print *,'   lwork must be at least m'
         info = 21
      endif

      if ( info .ne. 0 ) then
         call xerbla('BSCSM', info)
         return
      endif
...
c
      ...
      call dbscsmk( transpose, mb, n, scale, dv, dv, alpha, uplo, diag, 
     *     val, bindx, bpntrb, bpntre, lb,
     *     b, ldb, beta, c, ldc, work, lwork)
...
c--------------------------------------------------------------------
     ...
     *           b, ldb, beta, c, ldc, work, base)
      ...
      integer i,ii,j,bl,jb,je,vb,db,cb,bb,nb,lbsq,blkind,lwork
...
c         Working with transpose of lower triangular (== upper)
...
c         Working with the transpose of upper trianular (== lower)
          vb = 1 - lbsq
          db = 1 - lbsq
          bb = 1 - lb
          do 75 j=1,mb
