...
C
      ...
      REAL             BASIS(LDB,NQ),S(JT,NX),X(LNI,NX)
C
C==== local variables ==================================================
C 
      INTEGER          I,J
...
C
              DO 20 I = 1,NX
                 DO 10 J = 1,NQ
                    CALL SAXPY (NI,S(J,I),BASIS(1,J),1,X(1,I),1)
   10            CONTINUE
   20         CONTINUE
C
...
C
              DO 30 I = 1,NX
                 CALL SGEMV ('N',NI,NQ,ONE,BASIS,LDB,
     &                       S(1,I),1,ONE,X(1,I),1)
   30         CONTINUE
C
...
C
              CALL SGEMM ('N','N',NI,NX,NQ,ONE,BASIS,LDB,S,JT,ONE,X,LNI)
C
      END IF 
C
      RETURN
