...
c              CHERAMGinit         init default parameter  
c              CHERAMGfactor       compute multilevel ILU `PREC'
c              CHERAMGsol          solve a single linear system with `PREC'
c              CHERAMGsolver       solve linear system Ax=b iteratively
c                                  using the ILUPACK preconditioner
c              CHERAMGdelete       release memory
c              CHERAMGnnz          logical number of nonzero entries of ILU
c              CHERAMGinfo         display multilevel structure
c
      integer  CHERAMGfactor, CHERAMGsolver, CHERAMGnnz
      external CHERAMGinit,   CHERAMGsol, CHERAMGdelete,
     +         CHERAMGfactor, CHERAMGsolver,
     +         CHERAMGnnz, CHERAMGinfo
c
...
c     init default parameters  
      call CHERAMGinit(n,ia,ja,a,matching,ordering,
...
c     solve a single linear system with `PREC'
      call CHERAMGsol(param,PREC,rhs,sol,n)
c
...
c     solve Ax=b iteratively
      ierr=CHERAMGsolver(param,PREC,rhs,sol,
     +                   n,ib,jb,b,matching,ordering,
     +                   droptol, droptolS, condest, restol,
     +                   maxit, elbow, lfil, lfilS, nrestart, 
     +                   mixedprecision, ind)
