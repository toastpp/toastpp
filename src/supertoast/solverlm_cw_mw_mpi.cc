...
void GenerateJacobian (RFwdSolverMW &FWS, const Raster &raster,
    const QMMesh *mesh, const MWsolution &sol,
    const RCompRowMatrix &qvec, const RCompRowMatrix &mvec,
    DataScale dscale, RDenseMatrixMPI &J, const RDenseMatrix &excoef);

RCompRowMatrix BuildRHessian (Regularisation *reg, const RVector &x,
    int nprm);

RVector RescaleHessian (const  RDenseMatrixMPI &J, const RVector &x,
...
struct HESS_DATA {
    const RDenseMatrixMPI *J;    // Jacobian
...
static RVector JTJx_clbk (const RVector &x, void *context)
{
    ...
    const RDenseMatrixMPI *J    =  data->J;
    const RCompRowMatrix *RHess =  data->RHess;
    const RVector &M            = *data->M;
    const double lambda         = *data->lambda;
    int m = J->nRows(), n = J->nCols();
    ...
    const RCompRowMatrix &qvec, const RCompRowMatrix &mvec)
{
    ...
    RVector proj(ndat);
    RVector prmscl;
    RDenseMatrixMPI J(nqm*nlambda,slen*nprm);
    RMatrix *cov;
...
//    }
	...
	J.ColScale (bsol.GetActiveParams());
    ...
    DataScale dscale, RDenseMatrixMPI &J, const RDenseMatrix &excoef)
{
    ...
    J.New(nqm*nlambda,slen*sol.nActive());
	...
	for (q = 0; q < nq; q++) {
		...
		if (idx >= r0 && idx < r1) { // row in range of process
		    ...
		    if (!lambda_ok) {
			FWS.Reset (*sol.swsol[i]);
			if (bFactorA) {
			    gA.New (glen);
			    raster.Map_MeshToGrid (sol.GetJacobianCoeff_A(i),
						   gA);
			}
			if (bPowerb) {
			    gb.New (glen);
			    raster.Map_MeshToGrid (sol.GetJacobianCoeff_b(i),
						   gb);
			}
			lambda_ok = true;
		    }
			...
			if (bScatter) {
			    if (!cdfield_grad) cdfield_grad = new RVector[dim];
		    ...
		    for (c = 0; c < nchromo; c++) {
			for (k = 0; k < slen; k++) {
			    J(idx,c*slen+k) = pmdf_basis[k]*excoef(i,c);
			}
		    }
			...
			if (dscale == DATA_LOG)
			    pmdf = PMDF_log (pmdf, proj[mm]);
			if (bFactorA) {
			    raster.Map_GridToSol (pmdf*gA, pmdf_basis);
			    for (k = 0; k < slen; k++)
				J(idx,c*slen+k) = pmdf_basis[k];
			    c++;
			}
			if (bPowerb) {
			    raster.Map_GridToSol (pmdf*gb, pmdf_basis);
			    for (k = 0; k < slen; k++)
				J(idx,c*slen+k) = pmdf_basis[k];
			    c++;
			}
