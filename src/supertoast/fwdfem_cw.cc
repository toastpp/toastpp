...
int main (int argc, char *argv[]) {
	...
	case DATA_LIN:
	    OpenNIM ("phi.nim", meshname, n);
	    for (i = 0; i < nQ; i++) {
	        for (j = 0; j < n; j++) buf[j] = dphi[i][j];
	        WriteNIM ("phi.nim", buf, n, i);
	    }
	    cout << "  Real field written to phi.nim" << endl;
	    break;
	case DATA_LOG:
	    OpenNIM ("lphi.nim", meshname, n);
	    for (i = 0; i < nQ; i++) {
	        for (j = 0; j < n; j++) buf[j] = log(dphi[i][j]);
	        WriteNIM ("lphi.nim", buf, n, i);
	    }
