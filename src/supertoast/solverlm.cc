...
#ifdef FDBG
	    if (inr == 1) {
	    // build a little perturbation vector
	    RVector pert_alpha(nlen);
	    Point cnt(dim);
	    for (i = 0; i < dim; i++) cnt[i] = 10.0;
	    double rad = 2.0;
	    for (i = 0; i < nlen; i++)
		if (cnt.Dist (mesh.nlist[i]) > rad)
		    pert_alpha[i] = 1e-6;
	    RVector pert_p(slen*2);
	    RVector pert_alpha_p(pert_p, 0, slen);
	    raster.Map_MeshToSol (pert_alpha, pert_alpha_p);

	    ofstream ofs1("dbg1.dat");
	    ofs1 << Ax(*J, pert_p) << endl;
	    ofstream ofs2("dbg2.dat");
	    ofs2 << FrechetDerivative (mesh, raster, FWS, mvec, dphi, aphi,
				       sd, prmscl, pert_p) << endl;
	    ofstream ofs3("dbg3.dat");
	    ofs3 << FrechetDerivative (mesh, raster, FWS, mvec, dphi,
				       sd, prmscl, pert_p) << endl;
	    exit (0);
	    }
#endif
...
//#define AFDBG
	    ...
	    ofstream ofs1("dbg1.dat");
	    ofs1 << ATx(*J, data) << endl;
	    ofstream ofs2("dbg2.dat");
	    ofs2 << AdjointFrechetDerivative (mesh, raster, FWS, mvec,
		 dphi, aphi, sd, prmscl, data) << endl;
	    ofstream ofs3("dbg3.dat");
	    ofs3 << AdjointFrechetDerivative (mesh, raster, FWS, mvec,
		 dphi, sd, prmscl, data) << endl;
	    exit (0);
	    }
#endif
		...
		M = ImplicitHessianScaling (*mesh, raster, FWS, mvec,
		    dphi, aphi_hscale, sd, prmscl);
		break;
	    ...
	    // calculate gradient
	    r = AdjointFrechetDerivative (*mesh, raster, FWS, mvec, dphi, aphi,
	        sd, b);
