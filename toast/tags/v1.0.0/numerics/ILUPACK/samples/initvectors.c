    ...
    if (nrhs==0) {
      ...
      if (transpose)
	 CSRMATTVEC(AT,sol+A.nr*l,rhs+A.nr*l);
      else
	 CSRMATVEC (A, sol+A.nr*l,rhs+A.nr*l);
    ...
    if (param.ndbuff<A.nr) {
       param.ndbuff=MAX(param.ndbuff,A.nr);
       param.dbuff=(FLOAT *)REALLOC(param.dbuff,(size_t)param.ndbuff,"main:param.dbuff");
    }
    if (transpose)
       CSRMATTVEC(AT,sol+A.nr*l,param.dbuff);
    else
       CSRMATVEC (A, sol+A.nr*l,param.dbuff);
