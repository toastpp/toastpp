...
c              ZGNLAMGinit         init default parameter  
c              ZGNLAMGfactor       compute multilevel ILU `PREC'
c              ZGNLAMGsol          solve a single linear system with `PREC'
c              ZGNLAMGsolver       solve linear system Ax=b iteratively
c                                  using the ILUPACK preconditioner
c              ZGNLAMGdelete       release memory
c              ZGNLAMGnnz          logical number of nonzero entries of ILU
c              ZGNLAMGinfo         display multilevel structure
c
      integer  ZGNLAMGfactor, ZGNLAMGsolver, ZGNLAMGnnz
      external ZGNLAMGinit,   ZGNLAMGsol, ZGNLAMGdelete,
     +         ZGNLAMGfactor, ZGNLAMGsolver,
     +         ZGNLAMGnnz, ZGNLAMGinfo
c
...
c     init default parameters  
      call ZGNLAMGinit(n,ia,ja,a,matching,ordering,
...
c     solve a single linear system with `PREC'
      call ZGNLAMGsol(param,PREC,rhs,sol,n)
c
...
c     solve Ax=b iteratively
      ierr=ZGNLAMGsolver(param,PREC,rhs,sol,
     +                   n,ib,jb,b,matching,ordering,
     +                   droptol, droptolS, condest, restol,
     +                   maxit, elbow, lfil, lfilS, nrestart, 
     +                   mixedprecision, ind)
