...
c   Toolkit interface:
c   dbsrmm -- block sparse row format matrix-matrix multiply
c  
...
c
        call dbsrmmgk( transpose,  mb, n, kb, alpha,
     *       val, bindx, bpntrb, bpntre, lb,
...
c
        call dbsrmmkk( transpose,  mb, n, kb, alpha,
     *       val, bindx, bpntrb, bpntre, lb,
     *       b, ldb, beta, c, ldc, descra(4))
        return
      else
...
c--------------------------------------------------------------------
      subroutine DBSRMMGK( trans, mb, n, kb, alpha, 
     *           val, bindx, bpntrb, bpntre, lb, 
...
c            bb = starting (point) column of b 
 ...
 5    continue
      ...
      subroutine DBSRMMSK( mb, n, kb, alpha, val, bindx, bpntrb,
...
c
 ...
 5    continue
      ...
      subroutine DBSRMMKK( trans, mb, n, kb, alpha, val, bindx, bpntrb,
     *                          bpntre, lb, b, ldb, beta, c, ldc, base)
      implicit none
      integer mb, n, kb, lb, ldb, ldc, base
      integer lbsq
...
c                 Reverse sign on alpha if working with the transpose
                  call dgemm('N','N',lb,k,lb,-alpha,val(vb),lb,
     *                                            b(vc,bb),ldb,
     *                                        one,c(cb,bb),ldc)
c
