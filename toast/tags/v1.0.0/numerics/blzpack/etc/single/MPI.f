...
C
     ...
     &                          LCOMM,STATUS,IINFO)
                 CALL MPI_RECV (Y(J),K,MPI_REAL            ,I,2,
     &                          LCOMM,STATUS,IINFO)
                 J = J + K
         ELSE IF ( PID.EQ.I ) THEN
                 CALL MPI_SEND (N,1,MPI_INTEGER         ,0,1,
     &                          LCOMM,IINFO)
                 CALL MPI_SEND (X,N,MPI_REAL            ,0,2,
     &                          LCOMM,IINFO)
         END IF
...
C
      IF      ( FUNC .EQ. 'MAX' ) THEN
              CALL MPI_ALLREDUCE (X,Y,N,MPI_REAL            ,MPI_MAX,
     &                            LCOMM,IINFO)
      ELSE IF ( FUNC .EQ. 'MIN' ) THEN
              CALL MPI_ALLREDUCE (X,Y,N,MPI_REAL            ,MPI_MIN,
     &                            LCOMM,IINFO)
      ELSE IF ( FUNC .EQ. 'SUM' ) THEN
              CALL MPI_ALLREDUCE (X,Y,N,MPI_REAL            ,MPI_SUM,
     &                            LCOMM,IINFO)
      ELSE
              INFO = 1
      END IF
