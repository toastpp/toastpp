...
c              ZHERAMGinit         init default parameter  
c              ZHERAMGfactor       compute multilevel ILU `PREC'
c              ZHERAMGsol          solve a single linear system with `PREC'
c              ZHERAMGsolver       solve linear system Ax=b iteratively
c                                  using the ILUPACK preconditioner
c              ZHERAMGdelete       release memory
c              ZHERAMGnnz          logical number of nonzero entries of ILU
c              ZHERAMGinfo         display multilevel structure
c
      integer  ZHERAMGfactor, ZHERAMGsolver, ZHERAMGnnz
      external ZHERAMGinit,   ZHERAMGsol, ZHERAMGdelete,
     +         ZHERAMGfactor, ZHERAMGsolver,
     +         ZHERAMGnnz, ZHERAMGinfo
c
...
c     init default parameters  
      call ZHERAMGinit(n,ia,ja,a,matching,ordering,
...
c     solve a single linear system with `PREC'
      call ZHERAMGsol(param,PREC,rhs,sol,n)
c
...
c     solve Ax=b iteratively
      ierr=ZHERAMGsolver(param,PREC,rhs,sol,
     +                   n,ib,jb,b,matching,ordering,
     +                   droptol, droptolS, condest, restol,
     +                   maxit, elbow, lfil, lfilS, nrestart, 
     +                   mixedprecision, ind)
