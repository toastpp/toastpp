...
ostream& operator<< (ostream& o, OptimiZZZ& TF)// Exporting the results
{
  int Nove= TF.surfnet.NoVertices();   
  o << Nove<<endl;// Writing the number of Vertices  
  for(int j=0;j<Nove;j++)
    {
      o<< TF.ParaVec[j][0]<<endl;// Writing thita
    }
  o<<endl;
  for(int j=0;j<Nove;j++)
    {
      o<< TF.ParaVec[j][1]<<endl;// Writing Fi
    }
  return o;
}

istream& operator>> (istream& i, OptimiZZZ& TF)// Exporting the results
{
  int Nove= TF.surfnet.NoVertices();   
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 cout<<"No of inequalities = "<<NofIneq((RVector *) 0,0)<<endl;
  ...
  for (Iter=0;Iter<NoIter;Iter++)
    {
     ...
     Springs();
      ...
      cout<<"--------------------------------------------------------------------------------------"<<endl;
      cout<<"! Iter # "<<Iter<<" F_Cost= "<<Fobj_calc((RVector *) 0,0)<<" C_Cost= "
	  <<CVecConstruction((RVector *) 0,0)<<"Noineq = "<<NofIneq((RVector *) 0,0)<<endl;
      cout<<"--------------------------------------------------------------------------------------"<<endl;
      cout<<endl;
...
//************************************************************
 ...
 cout<<"C: ";
 for(int k=0;k<NoIter+1;k++)cout<<MonitorCost[k]<<" , ";
 cout<<endl<<endl<<"F: ";
 for(int k=0;k<NoIter+1;k++)cout<<MonitorF[k]<<" , ";
 cout<<endl<<endl<<"P: ";
 for(int k=0;k<NoIter;k++)cout<<MonitorP[k]<<" , ";
 cout<<endl<<endl<<"convec: ";
 for(int k=0;k<NoFac;k++)cout<<ConVec[k]<<" , ";
 ...
 cout<<endl<<endl;
 cout<< " Thita = {";
 for(int i = 0; i < NoVRT; i++)cout<<ParaVec[i][0]<<" , ";
 cout<<" } "<<endl<<endl;
 cout<< " Fi = {";
 for(int i = 0; i < NoVRT; i++)cout<<ParaVec[i][1]<<" , ";
 cout<<" } "<<endl;
}//end of Function 
...
void OptimiZZZ::F_min(void)
{
 ...
 JConVec.Transpone();
  ...
  while(GoBack==1)
  {
    ...
    cout<< "(((((((((  a=  "<<a<<"   step = "<<step<<" steplarge= "<<steplarge<<endl;
    ...
    cout<<"+------------------------------------------------------------------------------"<<endl;
    cout<<"|  Lan  <-: "<< Lleft<<"     :-|-: "<<Lcentral<<"      :-> "<<Lright<<endl;
    cout<<"|  LaC  <-: "<< LaCleft<<"   :-|-: "<<LaCcentral<<"    :-> "<<LaCright<<endl;
    cout<<"|   C   <-: "<< CostLeft<<"  :-|-: "<<CostCentral<<"     :-> "<<CostRight<<endl;
    cout<<"|   F   <-: "<< Fleft<<"  ```:-|-: "<<Fcentral<<"   `  :-> "<<Fright<<endl;
 ...
 */
    ...
    if(Next1Right==1 && Next2==1){a = a + step; cout<<"---- Right ----"<<endl; ToloIneq=0;}
    if(Next1Left==1 && Next2==-1){a = a - step; cout<<"---- Left ----"<<endl; ToloIneq=0;}
...
// CG(AtA, dv, Cv2, tolcg);//The solver
 ...
 //******************************************** 
	...
	cout <<" | "<< iN <<" |  C= " <<CCost2<<" |  inEq = "<<NiQ2<<" |  F = "<<Fobj_calc(XVec,NoVRT)<<endl;
