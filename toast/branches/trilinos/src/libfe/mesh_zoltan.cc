    ...
    : QMMesh ()
{
    ...
    lids = NULL;
    nodeType = NULL;
    nodeGlobalToLocal = NULL;
    nel = 0;
}

zoltanMesh::zoltanMesh (const Teuchos::RCP<const Teuchos::Comm<int> >& comm)
: QMMesh ()
{
    rank = comm->getRank();
    nproc = comm->getSize();
...
zoltanMesh::~zoltanMesh ()
{
    ...
    if (nodeGlobalToLocal) delete []nodeGlobalToLocal;
}

void zoltanMesh::Setup (bool mark_boundary)
{
...
void zoltanMesh::NodeToElementMap (int **_nndel, int ***_ndel) const
{
    *_nndel = nndel;
    *_ndel = ndel;
}

void zoltanMesh::ComputeNodeToElementMap (int **_nndel, int ***_ndel) const
{
    int i, j, n;
...
void zoltanMesh::ClearNodeToElementMap ()
{
    if (nndel) {
        delete []nndel;
	nndel = NULL;
...
void zoltanMesh::ComputeElementList (ZOLTAN_ID_TYPE *gid, int ngid,
...
void zoltanMesh::ComputeNodeTypeList (int **ntype) const
{
    int i, j, n = nlen();
...
void zoltanMesh::ComputeNodeMaps (int **glob2loc) const
{
    int i, j, n = nlen();
    int *g2l = new int[n];
    for (i = j = 0; i < n; i++) {
...
void zoltanMesh::ComputePartition (GraphData *graph)
{
...
void zoltanMesh::Partition (struct Zoltan_Struct *zz)
{
  int i, rc;
...
void zoltanMesh::Migrate (struct Zoltan_Struct *zz)
{
    int rc;
    ...
    if (nodeGlobalToLocal) delete []nodeGlobalToLocal;
    ComputeNodeMaps (&nodeGlobalToLocal);
}

// ============================================================================

void zoltanMesh::AddToElMatrix (int el, CCompRowMatrixMPI &M, RVector *coeff,
    int mode) const
{
    int i, j, is, js, nnode;
...
void zoltanMesh::AddToSysMatrix (CCompRowMatrixMPI &M, RVector *coeff, int mode)
    const
{
    int i;
    for (i = 0; i < nel; i++) {
...
void zoltanMesh::CreateZoltanDictionary ()
{
...
int zoltanMesh::Zoltan_get_number_of_vertices (void *data, int *ierr)
{
...
void zoltanMesh::Zoltan_get_vertex_list (void *data, int sizeGID, int sizeLID,
...
void zoltanMesh::Zoltan_get_num_edges_list (void *data, int sizeGID,
    int sizeLID, int num_obj, ZOLTAN_ID_PTR globalID, ZOLTAN_ID_PTR localID,
...
void zoltanMesh::Zoltan_get_edge_list(void *data, int sizeGID, int sizeLID,
...
void zoltanMesh::Zoltan_get_message_sizes(void *data, int gidSize, int lidSize,
...
void zoltanMesh::Zoltan_pack_object_messages (void *data, int gidSize,
    int lidSize, int num_ids, ZOLTAN_ID_PTR globalIDs, ZOLTAN_ID_PTR localIDs, 
...
void zoltanMesh::Zoltan_unpack_object_messages (void *data, int gidSize,
...
void zoltanMesh::Zoltan_mid_migrate (void *data, int gidSize, int lidSize,
