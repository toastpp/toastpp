...
void GenerateJacobian (const Raster &raster, const CVector *dphi,
    const CVector *aphi, Measurement meas, RDenseMatrix &J, const RVector &sd,
    const RVector &kap, RVector *colsum = 0);
...
bool LineSearch (FwdSolver &FWS, const Raster &raster, const Scaler *pscaler,
...
void SolverPCG::Solve (FwdSolver &FWS, const Raster &raster,
    ...
    double ftol)
{
	...
	GenerateJacobian (raster, dphi, aphi, FWS.datatype, J, sd, kap);
	J.ATx (b,r);
	r = -r;
	raster.Map_SolToGrid (r1, rg);
    ...
    LOGOUT_3PRM ("Iteration 0  CPU %f  OF %f  (prior %f)",
        (double)(tm.tms_utime-clock0)/(double)HZ, of, of_prior);
	...
	LOGOUT_4PRM ("Iteration %d  CPU %f  OF %f  (prior %f)",
	    i_count, (double)(tm.tms_utime-clock0)/(double)HZ, of, of_prior);
