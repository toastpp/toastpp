...
Solver_CW_MW_MPI *Solver_CW_MW_MPI::Create (SOLVER solver)
{
    switch (solver) {
    case SOLVER_PCG: return new SolverPCG_CW_MW_MPI; break;
    case SOLVER_LM:  return new SolverLM_CW_MW_MPI; break;
    default:         return 0;
    }
}

Solver_CW_MW_MPI *Solver_CW_MW_MPI::Create (ParamParser *_pp)
{
    char cbuf[256];
    Solver_CW_MW_MPI *s = 0;

    if (_pp->GetString ("SOLVER", cbuf)) {
	if (!strcasecmp (cbuf, "PCG"))
	    s = new SolverPCG_CW_MW_MPI (_pp);
	else if (!strcasecmp (cbuf, "LM"))
	    s = new SolverLM_CW_MW_MPI (_pp);
	...
	cin >> cmd;
	switch (cmd) {
	case 1: s = new SolverPCG_CW_MW_MPI (_pp); break;
	case 2: s = new SolverLM_CW_MW_MPI (_pp); break;
