...
int main (int argc, char *argv[])
{
    ...
    FCompRowMatrix *smat = FWS.F;
    FCompRowMatrix *mmat = FWS.B;
    FCompRowMatrix K0 = -(*smat*(1-theta) - *mmat*(1.0/dtime));
    FCompRowMatrix K1 = *smat*theta + *mmat*(1.0/dtime);
    ...
    wc0 = time(NULL);
    FVector qi(n);
    FPreconditioner *precon = 0;
    for (i = 0; i < nQ; i++) {
    	double tol = lin_tol;
	FVector q = qvec.Row(i) * (1.0/dtime);
    	IterativeSolve (K1, q, dphi[i], tol, precon, maxit);
    }
    FVector proj(tpsf,0,nQM);
	...
	FVector proj(tpsf,step*nQM,nQM);
	proj = FWS.ProjectAll (mvec, dphi);
