...
c              DGNLAMGinit         init default parameter  
c              DGNLAMGfactor       compute multilevel ILU `PREC'
c              DGNLAMGsol          solve a single linear system with `PREC'
c              DGNLAMGsolver       solve linear system Ax=b iteratively
c                                  using the ILUPACK preconditioner
c              DGNLAMGdelete       release memory
c              DGNLAMGnnz          logical number of nonzero entries of ILU
c              DGNLAMGinfo         display multilevel structure
c
      integer  DGNLAMGfactor, DGNLAMGsolver, DGNLAMGnnz
      external DGNLAMGinit,   DGNLAMGsol, DGNLAMGdelete,
     +         DGNLAMGfactor, DGNLAMGsolver,
     +         DGNLAMGnnz, DGNLAMGinfo
c
...
c     init default parameters  
      call DGNLAMGinit(n,ia,ja,a,matching,ordering,
...
c     Just for fun: display multilevel information on the screen
     ...
     +      condest+0.005
      ...
      call DGNLAMGinfo(param,PREC,n,ib,jb,b)
c
...
c     solve a single linear system with `PREC'
      call DGNLAMGsol(param,PREC,rhs,sol,n)
c
...
c     solve Ax=b iteratively
      ierr=DGNLAMGsolver(param,PREC,rhs,sol,
     +                   n,ib,jb,b,matching,ordering,
     +                   droptol, droptolS, condest, restol,
     +                   maxit, elbow, lfil, lfilS, nrestart, 
     +                   mixedprecision, ind)
