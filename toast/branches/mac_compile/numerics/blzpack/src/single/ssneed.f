...
C
     ...
     &                 NSLS,NSRS,NSRLS,NSRRS,NVB
      REAL             BIGNUM,TIME(*),EPS,GRNRM,RNORM(JT),
     &                 THETA(JT),THETAL,THETAR,THRES
C
C==== local variables ==================================================
C
      INTEGER          I,INDEX,INFO,J,NRNRM,NTSOL
      REAL             C1,C2,W1,W2,W3,RSNRM,T(5),TBASIS,
...
C
      ...
      T(5) = TBASIS
C
      CALL PISRED ('SUM',5,T,TSUM,LCOMM,INFO)
C
      TFACTR = TSUM(1)/NPE
      TPURGE = TSUM(2)/NPE
      TEIGTB = TSUM(3)/NPE
