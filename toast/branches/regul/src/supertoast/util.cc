...
bool ReadPGM (RVector &img, IVector &gdim, const char *fname)
{
    char cbuf[256];
    bool binary;
    int i, nx, ny, nv, range;

    FILE *f = fopen (fname, "rb");
    if (!fgets (cbuf, 256, f)) return false;
    if (!strncasecmp (cbuf, "P2", 2)) {
        binary = false;
    } else if (!strncasecmp (cbuf, "P5", 2)) {
        binary = true;
    } else {
        xERROR ("Image format not recognised.");
        return false;
    }
    do {
      fgets(cbuf, 256, f);
	...
	if (range < 256) {
	    unsigned char *buf = new unsigned char[nv];
	    fread (buf, sizeof(unsigned char), nv, f);
	    for (i = 0; i < nv; i++) img[i] = (double)buf[i];
	    delete []buf;
	} else {
	    unsigned short *buf = new unsigned short[nv];
	    fread (buf, sizeof(unsigned short), nv, f);
	    for (i = 0; i < nv; i++) img[i] = (double)buf[i];
	    delete []buf;
	}
    }
    ...
    double *scalemin, double *scalemax, const char *fname)
{
    ...
    if (!have_colmap) {
        int r, g, b;
        char colormap[256];
	const char *rootdir = getenv("TOASTDIR");
	if (rootdir) {
	    strcpy (colormap, rootdir);
	} else {
	    strcpy (colormap, ".");
	    cerr << "WARNING: TOASTDIR environment variable not defined."
		 << endl;
	}
