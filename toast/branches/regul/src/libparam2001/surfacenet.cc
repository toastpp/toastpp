...
std::istream &operator>>  (std::istream& i, SurfaceNet &sn)//  ?
{
...
std::ostream& operator<< (std::ostream& o, SurfaceNet& sn)//   ?
{
  o<< sn.NeWNoVRT<<std::endl;
  for(int j=0;j<sn.NeWNoVRT;j++)o<< sn.VertexList[j]<<std::endl;
  o<< sn.LengoFaces<<std::endl;
  for(int j=0;j<sn.LengoFaces;j++)o<<sn.AntiClocFaces[j]<<std::endl;
  return o;
}
...
{
   std:: ofstream outFac(ofname);
  
  int NoFa= LengoFaces;
  
  outFac<<NoFa<<std::endl;
  
  for(int i = 0;i< NoFa ;i++)
    { 
      outFac<< AntiClocFaces[i] <<std::endl;
