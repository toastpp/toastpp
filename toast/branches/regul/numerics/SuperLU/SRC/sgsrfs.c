 ...
 *           i.e., U has types: Stype = NC, Dtype = _S, Mtype = TRU.
 *
...
#define ITMAX 5
    ...
    *info = 0;
    ...
    if ( !notran && !lsame_(trans, "T") && !lsame_(trans, "C"))	*info = -1;
	      ...
	      A->Stype != NC || A->Dtype != dtypeS || A->Mtype != GE )
	*info = -2;
    else if ( L->nrow != L->ncol || L->nrow < 0 ||
 	      L->Stype != SC || L->Dtype != dtypeS || L->Mtype != TRLU )
	*info = -3;
    else if ( U->nrow != U->ncol || U->nrow < 0 ||
 	      U->Stype != NC || U->Dtype != dtypeS || U->Mtype != TRU )
	*info = -4;
    else if ( ldb < MAX(0, A->nrow) ||
 	      B->Stype != DN || B->Dtype != dtypeS || B->Mtype != GE )
        *info = -10;
    else if ( ldx < MAX(0, A->nrow) ||
 	      X->Stype != DN || X->Dtype != dtypeS || X->Mtype != GE )
	*info = -11;
    if (*info != 0) {
	i = -(*info);
	xerbla_("sgsrfs", &i);
