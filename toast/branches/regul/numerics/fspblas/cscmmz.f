...
c--------------------------------------------------------------------
      subroutine dcscmmz( transa, m, n, k, alpha, descra,
...
c
        if (transpose .eq. 'N') then
          call dcscmmgkz( m, n, k, alpha,
     *       val, indx, pntrb, pntre,
     *       b, ldb, beta, c, ldc, descra(4))
        else
          call dcsrmmgkz( k, n, m, alpha,
     *       val, indx, pntrb, pntre,
     *       b, ldb, beta, c, ldc, descra(4))
        endif
        return
...
c--------------------------------------------------------------------
      subroutine DCSCMMGKZ( m, n, k, alpha, val, indx, pntrb,
...
c Symmetric variant:
      subroutine DCSCMMSKZ( m, n, k, alpha, val, indx, pntrb,
     *                          pntre, b, ldb, beta, c, ldc, base)
	  implicit none
      integer m, n, k, ldb, ldc, base
      integer maxcache, cacheline
...
c
             do 25 j=blkcolb,blkcole
                jb = pntrb(j)+1
                je = pntre(j)+1
                if ( indx(jb)+1 .eq. j ) then
                  c(j,l) = c(j,l) + alpha * b(j,l) * val(jb)
                  jb = jb + 1 
                elseif (indx(je-1)+1 .eq. j ) then
...
c Skew-Symmetric variant:
      subroutine DCSCMMKKZ( m, n, k, alpha, val, indx, pntrb,
     *                          pntre, b, ldb, beta, c, ldc, base)
	  implicit none
      integer m, n, k, ldb, ldc, base
      integer maxcache, cacheline
...
c
             do 25 j=blkcolb,blkcole
                jb = pntrb(j)+1
                je = pntre(j)+1
                if ( indx(jb)+1 .eq. j ) jb = jb + 1 
                if ( indx(je-1)+1 .eq. j ) je = je - 1 
                t = ddotiz(je-jb, val(jb), indx(jb), b(1,l))
                call daxpyiz(je-jb, alpha*b(j,l),val(jb),
     *               indx(jb),c(1,l))
                c(j,l) = c(j,l) - alpha * t
