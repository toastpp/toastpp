...
C
      IF      ( FUNC .EQ. 'MAX' ) THEN
              CALL PVMFREDUCE  (PVMMAX,Y,N,REAL4,1,GROUP,0,IINFO)
      ELSE IF ( FUNC .EQ. 'MIN' ) THEN
              CALL PVMFREDUCE  (PVMMIN,Y,N,REAL4,1,GROUP,0,IINFO)
      ELSE IF ( FUNC .EQ. 'SUM' ) THEN
              CALL PVMFREDUCE  (PVMSUM,Y,N,REAL4,1,GROUP,0,IINFO)
      ELSE
              INFO = 1
      END IF
C
...
C
      IF ( PID .EQ. 0 ) THEN
         CALL PVMFINITSEND (PVMDATARAW,IINFO)
         CALL PVMFPACK     (REAL4,Y,N,1,IINFO)
         CALL PVMFBCAST    (GROUP,8,IINFO)
      ELSE
         CALL PVMFRECV     (-1,8,IINFO)
         CALL PVMFUNPACK   (REAL4,Y,N,1,IINFO)
      END IF
C
