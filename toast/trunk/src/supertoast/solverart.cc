    ...
    clbkART_Ai Ai, void *context)
{
    ...
    Regularisation *reg = ((ART_DATA*)context)->reg;
    RCompRowMatrix *RHess = ((ART_DATA*)context)->RHess;

    if (reg && RHess) {
	Rgrad = reg->GetGradient(x);
    } else {
	reg = 0;
	RHess = 0;
    }
	...
	i = perm[ii];
	RVector ai = Ai(i, J);
	rhi = (reg ? (*RHess)(i,i) : 0.0);

	x += lambda/((ai & ai) + rhi) * (ai * (b[i] - (ai & x)) - Rgrad[i]);

	    //x += ai * ((b[i] - (ai & x)) / ((ai & ai) + rhi) * lambda);
    }
}
