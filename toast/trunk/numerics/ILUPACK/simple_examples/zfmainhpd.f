...
c     right hand side and approximate solution
      doublecomplex rhs(nnz)
      doublecomplex sol(nnz)
c
c     auxiliary variables
      integer         i,j,mem
c
c     external ILUPACK functions
c              ZHPDAMGinit         init default parameter  
c              ZHPDAMGfactor       compute multilevel ILU `PREC'
c              ZHPDAMGsol          solve a single linear system with `PREC'
c              ZHPDAMGsolver       solve linear system Ax=b iteratively
c                                  using the ILUPACK preconditioner
c              ZHPDAMGdelete       release memory
c              ZHPDAMGnnz          logical number of nonzero entries of ILU
c              ZHPDAMGinfo         display multilevel structure
c
      integer  ZHPDAMGfactor, ZHPDAMGsolver, ZHPDAMGnnz
      external ZHPDAMGinit,   ZHPDAMGsol, ZHPDAMGdelete,
     +         ZHPDAMGfactor, ZHPDAMGsolver,
     +         ZHPDAMGnnz,    ZHPDAMGinfo
c
...
c     init default parameters  
      call ZHPDAMGinit(n,ia,ja,a,matching,ordering,
...
c     solve a single linear system with `PREC'
      call ZHPDAMGsol(param,PREC,rhs,sol,n)
c
...
c     solve Ax=b iteratively
      ierr=ZHPDAMGsolver(param,PREC,rhs,sol,
     +                   n,ib,jb,b,matching,ordering,
     +                   droptol, droptolS, condest, restol,
     +                   maxit, elbow, lfil, lfilS, nrestart, 
     +                   mixedprecision, ind)
