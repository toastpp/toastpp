...
C
      IF      ( FUNC .EQ. 'MAX' ) THEN
              CALL PVMFREDUCE  (PVMMAX,Y,N,INTEGER8,1,GROUP,0,IINFO)
      ELSE IF ( FUNC .EQ. 'MIN' ) THEN
              CALL PVMFREDUCE  (PVMMIN,Y,N,INTEGER8,1,GROUP,0,IINFO)
      ELSE IF ( FUNC .EQ. 'SUM' ) THEN
              CALL PVMFREDUCE  (PVMSUM,Y,N,INTEGER8,1,GROUP,0,IINFO)
      ELSE
              INFO = 1
      END IF
C
      IF ( PID .EQ. 0 ) THEN
         ...
         CALL PVMFPACK     (INTEGER8,Y,N,1,IINFO)
         CALL PVMFBCAST    (GROUP,8,IINFO)
      ELSE
         CALL PVMFRECV     (-1,8,IINFO)
         CALL PVMFUNPACK   (INTEGER8,Y,N,1,IINFO)
      END IF
C
