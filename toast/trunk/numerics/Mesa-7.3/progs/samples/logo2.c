...
float xRotation = 90.0, yRotation = 0.0;
...
static void Draw(void)
{
   glClearColor(.5, .5, .5, 0);
    ...
    glLoadIdentity();
    if (Ortho) {
       glOrtho(-5*AR, 5*AR, -5, 5, 1, 50);
    }
    else {
       gluPerspective(90, AR, 1.0, 50.0);
    }
...
#else
    {
       float radius = 1.0, height = 8.0;
       int slices = 14, stacks = 2;
       glPushMatrix();
       glTranslatef(Separation, 0, -0.5*height);
       gluCylinder(quadObj, radius, radius, height,
                   slices, stacks);
       glTranslatef(-2*Separation, 0, 0);
       gluCylinder(quadObj, radius, radius, height,
                   slices, stacks);
       glPopMatrix();
    }
